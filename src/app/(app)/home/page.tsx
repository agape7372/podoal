...
import { arrayMove, computeTargetIndex, shiftFor, rowFootprint, inferRowGap, type SlotSnapshot } from '@/lib/reorder';
...
export default function HomePage() {
  ...
  const onMove = (e: React.PointerEvent, board: BoardSummary) => {
    ...
    if (L && L.id === board.id) {
      ...
      // 삽입 인덱스가 바뀔 때만 이웃을 재배치한다(미드포인트 교차 시 1회) →
      // 그 순간에만 CSS transition으로 스르륵 비켜준다.
