...
> - **Motion conventions (2026-07-03)**: 이징은 `--ease-standard`(이동/진입)/`--ease-backout`(오버슈트 비트), duration은 `--motion-fast/base/slow` 토큰 사용. **신규 코드에 `transition-all` 금지** — 실제 변하는 속성만 명시(`transition-[transform,box-shadow]` 등). 런타임에 변하는 진행바는 `width`가 아닌 `w-full origin-left` + `transform: scaleX()`. 신규 키프레임은 고유 이름 필수(@theme 동명 가림 회귀 선례). 모달은 공용 `Modal`의 variant 프리셋(sheet/center)이 스타일·진입/이탈 애니를 주입 — 호출부는 `useModalClose(onClose)`의 `requestClose`로 닫아 이탈 애니를 거친다(프리셋 밖 외형은 `unstyled`).
...
> - **AI illustration set (2026-07-23, live)**: `public/illustrations/empty/*-v2.webp` + `*-v1.webp` (11 empty-state scenes) + `public/illustrations/tiers/tier-{1..7}-v2.webp` (winery tier badges) + `public/illustrations/labels/label-{health,growth,lifestyle,work,social,hobby,mental}-v1.webp` (wine varietal labels), transparent WebP. Wired via `EmptyState`'s `art` prop (9 pages) and winery `TIER_ART`/`LABEL_ART` (both use the same art↔`EmojiIcon`-fallback badge pattern — `TierBadge`/`LabelBadge` in `winery/page.tsx`). Labels key off `templateId.split('-')[0]`, same convention as `WineBottle.tsx`'s `VARIETAL_FOIL`. **Style源 = `docs/ILLUSTRATION_STYLE.md` v2 "플랫 이모지"** (flat 2D vector, NOT clay-3D). Generation: Higgsfield `z_image`; background removal: local rembg pipeline (`scripts/art-tile.mjs` header documents it). **Motif rule (2026-07-23)**: z_image reliably regresses to glossy 3D/isometric render for discrete-object subjects (house, book, palette, moon) no matter the negative prompting — a unified "grape cluster + small color accent" motif avoids this (fruit shapes are flat-emoji-biased in the model). Prefer that motif for any new category icon/empty-state art. Replacing any asset = bump `-vN` suffix + update references (sw.js caches images cache-first — never overwrite same URL). **Exception: `src/app/opengraph-image.png`** is replaced same-path (SNS crawlers fetch it directly, not through the SW) — current OG (2026-07-07) = flat vignette + MaruBuri typography, 합성 소스 보존 `art-intake/og-compose.html`.
...
src/app/(app)/
  ...
  notifications/inbox/ # 알림함 — the unified feed page (GET /api/notifications, see more/ note)
  profile/           # Profile: name/avatar edit, password change (email accounts), 2-step
                     #   account deletion (transactional cascade), logout (hard redirect)
...
**Standalone static reference pages** (not Next routes, not shipped): `docs/anim-pick.html` (final grape-fill animation candidates 40·49·17, particle-free) — moved out of `public/` so it no longer ships in the prod bundle; kept under `docs/` as a historical design reference. The leaf candidate galleries (`leaf-options.html`, `leaf-options-v2.html`) were removed once the grape leaf was finalized (current trace shape, sage `#74A77E`). The earlier `anim-lab.html` (68 candidates) and the `(app)/animation-test` route were also removed.
