...
> - **AI illustration set (2026-07-06, live)**: `public/illustrations/empty/*-v2.webp` (8 empty-state scenes) + `public/illustrations/tiers/tier-{1..7}-v2.webp` (winery tier badges), transparent WebP. Wired via `EmptyState`'s `art` prop (6 pages) and winery `TIER_ART`. **Style源 = `docs/ILLUSTRATION_STYLE.md` v2 "플랫 이모지"** (flat 2D vector, NOT clay-3D — the old clay preamble in that doc's history is dead). Generation: Higgsfield `z_image`; background removal: local rembg pipeline (`scripts/art-tile.mjs` header documents it). Replacing any asset = bump `-vN` suffix + update references (sw.js caches images cache-first — never overwrite same URL). **Exception: `src/app/opengraph-image.png`** is replaced same-path (SNS crawlers fetch it directly, not through the SW) — current OG (2026-07-07) = flat vignette + MaruBuri typography, 합성 소스 보존 `art-intake/og-compose.html`.
