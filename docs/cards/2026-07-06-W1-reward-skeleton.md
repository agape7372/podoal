상태: 검증대기 (구현·검증 완료 2026-07-06 — 800ms 지연 시뮬: 조기 탭 스켈레톤 750ms(1왕복, '저장 중' 문구 표시)→본문 ✓ / 무탭 완성 자동 오픈(+2.4s) 스켈레톤 없이 본문 ✓ / 재열람 즉시 본문·reveal 영속 ✓ / 적대 검증 반례 1건(열고 닫은 뒤 자동 재개봉) 발견·rewardSeenRef 가드로 봉합 ✓. 스펙 5(친구 뷰)는 수정 불필요로 판명 — 보상 섹션이 렌더부터 isOwner 전용이고 선물 복사본은 ownerId=수령자라 전 케이스 정합(의도적 동작 기록). W2-A 병합분(fillBoard 위치 문구·board GET myPlantedGifts additive·BoardDetail 타입) 포함 완료)
