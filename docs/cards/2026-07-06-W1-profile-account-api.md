상태: 완료 (2026-07-06 — 검증 상세는 아래 이력 참조)
...
- 탈퇴 2계정: 심기 201·선물 201·메시지 201 후 DELETE 200 → 재로그인 401, 상대 화면 4종 200, 선물 복사본 생존+giftedFrom null, 친구·메시지 정리 ✓.
- 비번: 오현재 403 · 6자미만 400 · 변경 200 → 구비번 401 / 신비번 200 ✓. logout 200 → me 401 ✓.
- 발견: POST /api/auth/me가 이미 로그아웃 역할(레거시) — 전용 라우트는 buildClearAuthCookie 재사용, me POST는 계약 유지.
- 판명: 소유 이전 불필요(복사본 ownerId=수령자) — giftedFromId null 절단만. 방어 삭제 1건(타인 보드 위 내 스티커, 정상 데이터면 0행).
