...
- 검증법 (실행 명령 포함)
- 보고 전 자가검증: 각 주장을 이 세션의 도구 결과와 대조 — 증거를 가리킬 수 있는 작업만
  보고. 검증 안 된 것은 "미검증"으로 명시. 테스트 실패는 출력과 함께, 건너뛴 단계는
  건너뛰었다고 보고. 완료·검증된 것은 헤징 없이 완료로.
...
**⚠ 모든 보고는 가설.** 2026-07-05 감사에서 탐색 에이전트의 "High 2건"(messages 스팸·NotificationSetting skip)이 재검증에서 전부 무효(과대/허위)였다. 카드 템플릿의 "보고 전 자가검증" 절이 1차 방어(허위 보고 원천 감소 — 공식 Fable 프롬프팅 가이드의 검증 블록, Anthropic 테스트에서 조작 상태보고 거의 제거), 상위 모델 재확인은 그대로 유지. 서브에이전트 diff·탐색 보고는 상위 모델이 **코드로 재확인** 후 채택. 리뷰 게이트는 `docs/REVIEW_CHECKLIST.md` 7항. 반려 2회 → 상위 모델, 3회 → 최상위 모델 직접(절차는 PRINCIPLES §10).
